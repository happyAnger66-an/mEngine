...
#include "m_engine/runtime/mEngineBuffers.h"
    ...
    : public ::testing::Test  // NOLINT(cppcoreguidelines-pro-type-member-init)
{
 protected:
  ...
  static constexpr std::string_view noDeviceSkipReason =
      "This test cannot run without any device present";
  static constexpr std::string_view noPoolSkipReason =
      "This test cannot be run against devices that do not support memory "
      "pools.";
};

TEST_F(mEngineBuffersTest, Stream) {
  if (mDeviceCount == 0) {
...
TEST_F(mEngineBuffersTest, CudaAllocator) {
...
TEST_F(mEngineBuffersTest, PinnedAllocator) {
  auto constexpr size = 1024;
...
TEST_F(mEngineBuffersTest, HostAllocator) {
  auto constexpr size = 1024;
...
TEST_F(mEngineBuffersTest, UVMAllocator) {
  auto constexpr size = 1024;
...
TEST_F(mEngineBuffersTest, CudaAllocatorAsync) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
  ...
  auto constexpr size = 1024;
  if (mMemPool == nullptr) {
    GTEST_SKIP() << "This test cannot be run against devices that do not "
...
void testBuffer(IBuffer& buffer, std::int32_t typeSize) {
  ...
  buffer.resize(size / 2);
  EXPECT_EQ(buffer.getCapacity(), size / 2);
  auto bufferWrapped = IBuffer::wrap(buffer.data(), buffer.getDataType(),
  ...
  auto byteBuffer = IBuffer::wrap(static_cast<std::uint8_t*>(buffer.data()),
  ...
  auto tensorWrapped = ITensor::wrap(
      buffer.data(), buffer.getDataType(),
  ...
  EXPECT_NO_THROW(tensorWrapped->reshape(
      ITensor::makeShape({static_cast<SizeType32>(buffer.getCapacity()) / 2})));
  EXPECT_THROW(tensorWrapped->reshape(ITensor::makeShape(
                   {static_cast<SizeType32>(buffer.getCapacity()) * 2})),
               std::bad_alloc);
}
}  // namespace

TEST_F(mEngineBuffersTest, DeviceBuffer) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
...
TEST_F(mEngineBuffersTest, DeviceTensor) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
  if (mMemPool == nullptr) {
...
TEST_F(mEngineBuffersTest, BufferSlice) {
  auto constexpr size = 1024;
  HostAllocator allocator{};
  ...
  EXPECT_EQ(static_cast<std::uint8_t*>(slice->data()) -
...
TEST_F(mEngineBuffersTest, BufferOutput) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
  if (mMemPool == nullptr) {
  ...
  for (std::size_t size : {0, 16}) {
    DeviceBuffer buffer{size, nvinfer1::DataType::kFLOAT, allocator};
    MENGINE_CUDA_CHECK(cudaMemsetAsync(buffer.data(), 0, buffer.getSizeInBytes(),
                                    streamPtr->get()));
    streamPtr->synchronize();
    std::stringstream ss;
    ss << buffer;
...
TEST_F(mEngineBuffersTest, TensorOutput) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
  if (mMemPool == nullptr) {
  ...
  nvinfer1::Dims constexpr dims{3, 16, 8, 4};
  CudaAllocatorAsync allocator{streamPtr, mMemPool};
  for (auto dataType : {nvinfer1::DataType::kFLOAT, nvinfer1::DataType::kHALF,
                        nvinfer1::DataType::kBOOL, nvinfer1::DataType::kINT8,
                        nvinfer1::DataType::kINT32, nvinfer1::DataType::kINT64,
                        nvinfer1::DataType::kUINT8}) {
    DeviceTensor tensor{dims, dataType, allocator};
    MENGINE_CUDA_CHECK(cudaMemsetAsync(tensor.data(), 0, tensor.getSizeInBytes(),
                                    streamPtr->get()));
    streamPtr->synchronize();
    std::stringstream ss;
    ss << tensor;
...
void testBufferType() {
  ...
  static_assert(dataType.isPointer() ||
                dataType.isUnsigned() != limits::is_signed);
  static_assert(
      std::is_same_v<T, typename DataTypeTraits<dataType.getDataType(),
                                                dataType.isUnsigned(),
                                                dataType.isPointer()>::type>);
  IBuffer::SharedPtr buffer{
      std::make_shared<HostBuffer>(size, dataType, allocator)};
...
TEST_F(mEngineBuffersTest, ExtendedTypes) {
  testBufferType<bool>();
  testBufferType<bool*>();
...
TEST_F(mEngineBuffersTest, BytesToString) {
...
TEST_F(mEngineBuffersTest, PinnedPoolAllocator) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
  ...
  {
    auto a = BufferManager::pinnedPool(ITensor::makeShape({512, 4, 4}),
                                       nvinfer1::DataType::kFLOAT);
    auto b = BufferManager::pinnedPool(ITensor::makeShape({512, 10}),
  ...
  std::size_t secondChunkSize;
  {
    ...
    auto c = BufferManager::pinnedPool(ITensor::makeShape({initChunkSize + 1}),
                                       nvinfer1::DataType::kUINT8);
    pool.logSegments();
    auto it = std::begin(segments);
    EXPECT_EQ(it->tag, nullptr);
...
TEST_F(mEngineBuffersTest, MemoryPool) {
  ...
  auto const initMemory = memCounters.getCpu();
  {
    ...
    EXPECT_EQ(memCounters.getCpu(), oneChunk);
    EXPECT_EQ(
        static_cast<std::uint8_t*>(ptr_1) - static_cast<std::uint8_t*>(ptr_0),
        alignment);
    EXPECT_EQ(
        static_cast<std::uint8_t*>(ptr_2) - static_cast<std::uint8_t*>(ptr_1),
        alignment);
  }
  EXPECT_EQ(memCounters.getCpu(), initMemory);
}

TEST_F(mEngineBuffersTest, PinnedPoolStressTest) {
  if (mDeviceCount == 0) {
    GTEST_SKIP() << noDeviceSkipReason;
  }
  ...
  std::mt19937 rnd{
      42};  // mersenne_twister_engine seeded with 42 NOLINT(*-msc51-cpp)
  ...
  for (auto const& [ptr, size] :
       BufferRange{allocations.data() + deallocIdx, deallocSize}) {
  ...
  EXPECT_GE(pool.getUsedSize(), poolUsedSize + totalUsedSize);
  EXPECT_EQ(memCounters.getPinned() - initMemory,
            pool.getReservedSize() - poolReservedSize);
  ...
  std::thread thread([&]() {
    ...
    std::shuffle(allocations.begin() + static_cast<std::ptrdiff_t>(deallocIdx),
                 allocations.end(), rnd);
