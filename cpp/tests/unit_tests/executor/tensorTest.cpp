...
void testCreateTensor(std::shared_ptr<tr::CudaStream> const& stream) {
  ...
  std::initializer_list<std::function<Tensor(Shape)>> const constructors = {
      &Tensor::cpu<T>, &Tensor::pinned<T>, &Tensor::managed<T>,
      [&stream](auto shape) { return Tensor::gpu<T>(stream, shape); }};
...
void testRoundTrip(std::shared_ptr<tr::CudaStream> const& stream) {
  ...
  for (std::size_t run = 0; run < 2; ++run) {
    ...
    if (numMismatches > 0) {
      MENGINE_LOG_WARNING("testRoundTrip mismatches for dtype %d: %lu",
                       static_cast<int>(dtype), numMismatches);
      continue;
    }
...
class ParamTest : public TensorTest,
...
TEST_P(ParamTest, serializeDeserialize) {
  ...
  switch (memoryType) {
    ...
    case MemoryType::kCPU_PINNED:
      tensor = cpuTensor.copyToPinned();
      break;
    case MemoryType::kUVM:
      tensor = cpuTensor.copyToManaged();
      break;
...
INSTANTIATE_TEST_SUITE_P(
    TensorTest, ParamTest,
    testing::Combine(testing::Values(MemoryType::kCPU, MemoryType::kCPU_PINNED,
                                     MemoryType::kUVM, MemoryType::kGPU)),
    generateTestName);
