...
class TensorView : virtual public ITensor, public BufferView {
 public:
  ...
  TensorView(ITensor::SharedPtr const& buffer, size_t offset, size_t size)
      : BufferView{buffer, offset * sizeDim0(*buffer),
                   size * sizeDim0(*buffer)},
        mDims{buffer->getShape()} {
    auto const dim0 = static_cast<size_t>(
        (mDims.nbDims >= 0 && mDims.d[0] >= 0) ? mDims.d[0] : 0);
    if (offset > dim0) {
  ...
  TensorView(IBuffer::SharedPtr const& buffer, size_t offset, size_t size,
             nvinfer1::Dims const& dims)
      : BufferView{buffer, offset, size}, mDims{dims} {
 ...
 private:
  static std::size_t sizeDim0(ITensor const& tensor) {
    ...
    return shape.nbDims > 0 && shape.d[0] > 0
               ? ITensor::volume(shape) / shape.d[0]
               : 0;
  }
