...
class BufferView : virtual public IBuffer {
 public:
  explicit BufferView(IBuffer::SharedPtr buffer, std::size_t offset,
                      std::size_t size)
      : mBuffer(std::move(buffer)), mOffset{offset}, mSize{size} {
    if (offset > mBuffer->getSize()) {
      throw std::out_of_range(std::string("offset ") + std::to_string(offset) +
                              " exceeds buffer size " +
  ...
  [[nodiscard]] size_t getSize() const override { return mSize; }
  ...
  void resize(std::size_t newSize) override {
    MENGINE_CHECK(newSize <= getCapacity());
    mSize = newSize;
  }

  void release() override { mSize = 0; }
