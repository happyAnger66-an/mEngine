...
std::string MemoryCounters::toString() const {
  return m_engine::common::fmtstr("[MemUsage] GPU %s, CPU %s, Pinned %s",
                                  bytesToString(this->getGpu()).c_str(),
                                  bytesToString(this->getCpu()).c_str(),
                                  bytesToString(this->getPinned()).c_str());
}

void MemoryCounters::allocate(MemoryType memoryType,
  ...
  switch (memoryType) {
    ...
    case MemoryType::kPINNEDPOOL:
      allocate<MemoryType::kPINNEDPOOL>(size);
      break;
    default:
...
void MemoryCounters::deallocate(MemoryType memoryType,
                                MemoryCounters::SizeType32 size) {
  switch (memoryType) {
    case MemoryType::kGPU:
    ...
    case MemoryType::kPINNED:
      ...
      break;
    case MemoryType::kPINNEDPOOL:
      deallocate<MemoryType::kPINNEDPOOL>(size);
      break;
    default:
      MENGINE_THROW("Unknown memory type");
  }
