...
static DataType convertDataTypeStrToEnum(std::string const& str) {
  if (str == "BOOL") return DataType::kBOOL;
  if (str == "I8") return DataType::kINT8;
  if (str == "I32") return DataType::kINT32;
  if (str == "I64") return DataType::kINT64;
  if (str == "U8") return DataType::kUINT8;
  if (str == "F16") return DataType::kHALF;
  if (str == "F32") return DataType::kFLOAT;
  if (str == "BF16") return DataType::kBF16;
  if (str == "F8_E4M3") return DataType::kFP8;
  MENGINE_THROW("Unsupported data type: " + str);
}
...
class SafeTensorArray : public INdArray {
 ...
 public:
  SafeTensorArray(std::shared_ptr<std::ifstream> const& fs,
                  std::string const& dtypeStr,
                  std::vector<int64_t> const& shape, int64_t offsetBegin,
                  int64_t offsetEnd)
      : mShape(shape),
  ...
  [[nodiscard]] DataType dtype() const override { return mDataType; }
};
...
class SafeTensor : public ISafeTensor {
 ...
 public:
  SafeTensor(char const* filename)
    ...
    if (!mFs->is_open()) {
      MENGINE_THROW("Failed to open file: " + std::string(filename));
    }
  ...
  std::shared_ptr<INdArray> getTensor(char const* name) override {
    ...
    if (it != mTensorInfo.end()) {
      ...
      return std::make_shared<SafeTensorArray>(
          mFs, value["dtype"], value["shape"],
          static_cast<int64_t>(value["data_offsets"][0]) + offset,
          static_cast<int64_t>(value["data_offsets"][1]) + offset);
    }
    MENGINE_THROW("Tensor not found: " + std::string(name));
  }
};
