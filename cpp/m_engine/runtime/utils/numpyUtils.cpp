...
namespace m_engine::runtime::utils
{
...
{
    MENGINE_LOG_DEBUG("numpy type: %s", type.c_str());
    ...
    MENGINE_CHECK_WITH_INFO(type_map.count(type) > 0, "numpy data type '" + type + "' not supported");
...
void parseNpyIntro(FILE*& f_ptr, uint32_t& header_len, uint32_t& start_data)
{
    ...
    MENGINE_LOG_DEBUG("npy format version: %d.%d", npy_major, npy_minor);
...
int parseNpyHeader(FILE*& f_ptr, uint32_t header_len, nvinfer1::DataType& type, std::vector<size_t>& shapeVec)
{
    ...
    MENGINE_LOG_DEBUG("npy header: %s", header.c_str());
    ...
    BufferManager const& manager, std::string const& npyFile, MemoryType const where)
{
    ...
    MENGINE_CHECK_WITH_INFO(statusCode == 0 && n_elems == size, "reading tensor failed");
...
void saveNpy(BufferManager const& manager, ITensor const& tensor, std::string const& filename)
{
    ...
    {
        MENGINE_CHECK(where == MemoryType::kGPU);
        ...
        mc::invokeCudaD2DcpyConvert(dataFp32, dataBf16, tensorSize);
    ...
    MENGINE_CHECK_WITH_INFO(f_ptr != nullptr, mc::fmtstr("Unable to open %s for writing.\n", filename.c_str()));
