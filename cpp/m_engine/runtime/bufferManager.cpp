...
void BufferManager::setMem(IBuffer& buffer, int32_t value) const
{
    ...
    {
        MENGINE_CUDA_CHECK(cudaMemsetAsync(buffer.data(), value, buffer.getSizeInBytes(), mStream->get()));
    }
    else
    {
...
{
    MENGINE_CHECK_WITH_INFO(src.getDataType() == dst.getDataType(),
        tc::fmtstr("Incompatible data types: %s != %s", src.getDataTypeName(), dst.getDataTypeName()));
    MENGINE_CHECK_WITH_INFO(src.getSizeInBytes() == dst.getSizeInBytes(),
...
void BufferManager::memoryPoolTrimTo(std::size_t size)
{
    if (!static_cast<bool>(mPool))
    {
        MENGINE_LOG_TRACE("Operation '%s' does not do anything on this system as it does not support memory pools.",
            __PRETTY_FUNCTION__);
        return;
    }
