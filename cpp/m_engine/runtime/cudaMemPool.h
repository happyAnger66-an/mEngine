...
class CudaMemPool {
 public:
  ...
  /// amount of reserved memory will actually decrease, only guarantees that
  /// this amount after trimming will be larger than the provided size.
  ...
  /// provided device ID if it was successfully initialized, nullptr otherwise.
  static std::shared_ptr<m_engine::runtime::CudaMemPool>
  getPrimaryPoolForDevice(int deviceId);
  ...
  /// systems, the UVM driver is explicitly disabled.
 ...
 private:
  ...
  using PoolPtr =
      std::unique_ptr<std::remove_pointer_t<cudaMemPool_t>, Deleter>;
