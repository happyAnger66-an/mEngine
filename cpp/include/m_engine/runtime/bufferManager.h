...
class BufferManager {
 public:
  ...
  //! (allocation, de-allocation, copying, etc.).
  ...
  //! cudaMallocAsync.
  [[nodiscard]] IBufferPtr gpu(std::size_t size,
                               nvinfer1::DataType type = kBYTE_TYPE) const;

  //! \brief Allocates an `ITensor` of the given dimensions on the GPU, using
  //! cudaMallocAsync.
  [[nodiscard]] ITensorPtr gpu(nvinfer1::Dims dims,
                               nvinfer1::DataType type = kBYTE_TYPE) const;

  //! \brief Allocates an `IBuffer` of the given size on the GPU, using
  //! cudaMalloc.
  [[nodiscard]] static IBufferPtr gpuSync(std::size_t size,
  ...
  //! default memory pool.
  [[nodiscard]] static IBufferPtr pinnedPool(
      std::size_t size, nvinfer1::DataType type = kBYTE_TYPE);

  //! \brief Allocates a pinned `ITensor` of the given dimensions on the CPU in
  //! the default memory pool.
  [[nodiscard]] static ITensorPtr pinnedPool(
      nvinfer1::Dims dims, nvinfer1::DataType type = kBYTE_TYPE);
  ...
  [[nodiscard]] static ITensorPtr ipcNvls(std::set<int> ranks,
                                          nvinfer1::Dims dims,
  ...
  //! resized later.
  [[nodiscard]] IBufferPtr emptyBuffer(
      MemoryType memoryType, nvinfer1::DataType type = kBYTE_TYPE) const {
    return allocate(memoryType, 0, type);
  }

  //! \brief Create an empty `ITensor` of the given memory type. It may be
  //! reshaped later.
  [[nodiscard]] ITensorPtr emptyTensor(
      MemoryType memoryType, nvinfer1::DataType type = kBYTE_TYPE) const {
  ...
  //! \brief Copy `src` into a new `IBuffer` with a potentially different memory
  //! type.
  [[nodiscard]] IBufferPtr copyFrom(IBuffer const& src,
                                    MemoryType memoryType) const;

  //! \brief Copy `src` into a new `ITensor` with a potentially different memory
  //! type.
  [[nodiscard]] ITensorPtr copyFrom(ITensor const& src,
                                    MemoryType memoryType) const;

  //! \brief Copy `src` into a new `IBuffer` with a potentially different memory
  //! type.
  template <typename T>
  [[nodiscard]] IBufferPtr copyFrom(std::vector<T> const& src,
                                    MemoryType memoryType) const {
    auto buffer = allocate(memoryType, src.size(),
                           TRTDataType<std::remove_cv_t<T>>::value);
  ...
  //! type.
  template <typename T>
  [[nodiscard]] ITensorPtr copyFrom(T* src, nvinfer1::Dims dims,
                                    MemoryType memoryType) const {
    auto buffer =
        allocate(memoryType, dims, TRTDataType<std::remove_cv_t<T>>::value);
    copy(src, *buffer);
    return buffer;
  }
  ...
  //! type.
  template <typename T>
  [[nodiscard]] ITensorPtr copyFrom(std::vector<T> const& src,
                                    nvinfer1::Dims dims,
                                    MemoryType memoryType) const {
    MENGINE_CHECK_WITH_INFO(
        src.size() == ITensor::volumeNonNegative(dims),
        common::fmtstr(
            "[M-ENGINE][ERROR] Incompatible size %lu and dims %s",
  ...
  //! synchronizes implicitly with the stream.
