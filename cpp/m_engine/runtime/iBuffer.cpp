...
#include "m_engine/runtime/bufferView.h"
...
MemoryType IBuffer::memoryType(void const* data)
{
    ...
    MENGINE_CUDA_CHECK(::cudaPointerGetAttributes(&attributes, data));
...
IBuffer::UniquePtr IBuffer::wrap(void* data, nvinfer1::DataType type, std::size_t size, std::size_t capacity)
{
    ...
    case MemoryType::kGPU:
        ...
        break;
    default: MENGINE_THROW("Unknown memory type");
    }
    result->resize(size);
    return result;
}

std::ostream& m_engine::runtime::operator<<(std::ostream& output, IBuffer const& buffer)
{
...
char const* IBuffer::getDataTypeName(DataType dataType)
{
    ...
    default: MENGINE_THROW("Unknown data type");
    }
}
