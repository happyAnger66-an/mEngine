...
namespace common {
#if ENABLE_BF16
static inline std::basic_ostream<char>& operator<<(
    std::basic_ostream<char>& stream, __nv_bfloat16 const& val) {
...
static inline std::basic_ostream<char>& operator<<(
    std::basic_ostream<char>& stream, __half const& val) {
...
void printTupleImpl(std::ostream& os, Tuple const& t,
...
void fmtstr_(char const* format, fmtstr_allocator alloc, void* target,
             va_list args);
...
template <typename T>
inline std::string vec2str(std::vector<T> const& vec,
                           char const* delim = kDefaultDelimiter) {
...
std::unordered_set<std::string> str2set(std::string const& input,
                                        char delimiter);
