...
using m_engine::common::fmtstr;
MmapReader::MmapReader() : mData(nullptr), mBytes(0) {}

MmapReader::MmapReader(std::filesystem::path const &fp)
    : mData(nullptr), mBytes(0) {
  if (!loadFile(fp)) {
    throw std::runtime_error("Failed to load file in MmapReader constructor");
  }
}

MmapReader::~MmapReader() { release(); }
...
bool MmapReader::loadFile(std::filesystem::path const &fp) {
  ...
  if (fd <= 0) {
    std::string errorMsg =
        fmtstr("MmapReader: Cannot open file: %s", filePath.c_str());
  ...
  if (fstat(fd, &status) != 0) {
    close(fd);
    std::string errorMsg =
        fmtstr("MmapReader: fstat failed for file: %s", filePath.c_str());
    std::cerr << errorMsg << std::endl;
    return false;
  }
  ...
  if (mBytes == 0) {
    close(fd);
    std::string errorMsg =
        fmtstr("MmapReader: File %s is empty.", filePath.c_str());
    std::cerr << errorMsg << std::endl;
    return false;
  }
  ...
  if (mData == MAP_FAILED) {
    ...
    close(fd);
    std::string errorMsg =
        fmtstr("MmapReader: mmap failed for file: %s", filePath.c_str());
    std::cerr << errorMsg << std::endl;
    return false;
  }
  close(fd);
...
void const *MmapReader::getData() const noexcept { return mData; }

size_t MmapReader::getSize() const noexcept { return mBytes; }
