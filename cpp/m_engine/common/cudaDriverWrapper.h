    ...
    T result, CUDADriverWrapper const& wrap, char const* const func, char const* const file, int const line)
{
    if (result)
    {
        ...
        throw mEngineException(file, line,
 ...
 */
#define MENGINE_CU_CHECK(stat)                                                                                            \
    do                                                                                                                 \
    {                                                                                                                  \
        m_engine::common::checkDriver(                                                                             \
            (stat), *m_engine::common::CUDADriverWrapper::getInstance(), #stat, __FILE__, __LINE__);               \
    } while (0)

// Avoid using CUDADriverWrapper when freeing resource, during which the global instance may already be freed.
#define MENGINE_CU_CHECK_FREE_RESOURCE(stat)                                                                              \
    do                                                                                                                 \
    {                                                                                                                  \
        m_engine::common::checkDriverExitSafe((stat), #stat, __FILE__, __LINE__);                                  \
    } while (0)
