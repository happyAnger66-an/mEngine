...
//#include "m_engine/runtime/ipcNvlsMemory.h"
#include "m_engine/runtime/memoryCounters.h"
#include "m_engine/runtime/virtualMemory.h"
...
class CudaAllocator : public BaseAllocator<CudaAllocator, MemoryType::kGPU> {
 ...
 protected:
  ...
  {
    MENGINE_CUDA_CHECK(::cudaMalloc(ptr, n));
  }

  void
  deallocateImpl(  // NOLINT(readability-convert-member-functions-to-static)
      PointerType ptr, [[maybe_unused]] std::size_t n) {
    MENGINE_CUDA_CHECK_FREE_RESOURCE(::cudaFree(ptr));
  }
};

class CudaAllocatorAsync
    : public BaseAllocator<CudaAllocatorAsync, MemoryType::kGPU> {
 ...
 public:
  ...
  explicit CudaAllocatorAsync(CudaStreamPtr stream, CudaPoolPtr memPool)
    ...
    MENGINE_CHECK_WITH_INFO(static_cast<bool>(mCudaStream),
                         "Undefined CUDA stream");
    MENGINE_CHECK_WITH_INFO(static_cast<bool>(mMemPool),
                         "Undefined CUDA mem pool");
  }
 ...
 protected:
  void allocateImpl(PointerType* ptr, std::size_t n) {
    MENGINE_CUDA_CHECK(
        ::cudaMallocFromPoolAsync(ptr, n, mMemPool->getPool(), mCudaStream->get()));
  }

  void deallocateImpl(PointerType ptr, [[maybe_unused]] std::size_t n) {
    MENGINE_CUDA_CHECK_FREE_RESOURCE(::cudaFreeAsync(ptr, mCudaStream->get()));
  }
...
class UVMAllocator : public BaseAllocator<UVMAllocator, MemoryType::kUVM> {
 ...
 protected:
  void allocateImpl(
      ...
      std::size_t n)  // NOLINT(readability-convert-member-functions-to-static)
  {
    MENGINE_CUDA_CHECK(::cudaMallocManaged(ptr, n));
    // MENGINE_CUDA_CHECK(::cudaMemAdvise(ptr, n,
...
class PinnedAllocator
    : public BaseAllocator<PinnedAllocator, MemoryType::kPINNED> {
 ...
 protected:
  void allocateImpl(
      ...
      std::size_t n)  // NOLINT(readability-convert-member-functions-to-static)
  {
    MENGINE_CUDA_CHECK(::cudaHostAlloc(ptr, n, cudaHostAllocDefault));
  }

  void
  deallocateImpl(  // NOLINT(readability-convert-member-functions-to-static)
      PointerType ptr, [[maybe_unused]] std::size_t n) {
    MENGINE_CUDA_CHECK_FREE_RESOURCE(::cudaFreeHost(ptr));
  }
};
...
class BorrowingAllocator
 ...
 public:
  ...
  BorrowingAllocator(void* ptr, std::size_t capacity)
    ...
    MENGINE_CHECK_WITH_INFO(capacity == std::size_t(0) || static_cast<bool>(mPtr),
                         "Undefined pointer");
  }
...
using PinnedPoolBorrowingAllocator =
    BorrowingAllocator<MemoryType::kPINNEDPOOL>;
...
class MemoryPool : public BaseAllocator<MemoryPool<TAllocator>,
                                        TAllocator::kMemoryType, false> {
  friend class BaseAllocator<MemoryPool<TAllocator>, TAllocator::kMemoryType,
                             false>;

 public:
  using Base =
      BaseAllocator<MemoryPool<TAllocator>, TAllocator::kMemoryType, false>;
  using PointerType = typename Base::PointerType;

  using Allocator = TAllocator;
  static_assert(std::is_same_v<typename Allocator::PointerType, PointerType>);

  static std::size_t constexpr kInitialChunkSize{std::size_t{1}
                                                 << 29};  // 512 MB
  static std::size_t constexpr kAlignment{256};

  explicit MemoryPool(std::size_t chunkSize = kInitialChunkSize,
                      Allocator allocator = Allocator{})
      : mChunkSize(chunkSize), mAllocator{allocator} {}

  ~MemoryPool() {
    std::lock_guard<std::mutex> lock(mLock);
    MENGINE_LOG_DEBUG("MemoryPool: Deallocating %zu chunks",
                   mAllocatedChunks.size());
    for (auto const& [ptr, size] : mAllocatedChunks) {
      MENGINE_LOG_DEBUG("MemoryPool: Deallocating %zu B", size);
      try {
        mAllocator.deallocate(ptr, size);
      } catch (std::exception const& e) {
        MENGINE_LOG_EXCEPTION(e);
      }
    }
  ...
  [[nodiscard]] std::size_t getUsedSize() const {
    std::lock_guard<std::mutex> lock(mLock);
    return std::accumulate(mMemorySegments.cbegin(), mMemorySegments.cend(),
                           std::size_t{0},
                           [](std::size_t sum, auto const& chunk) {
                             return chunk.tag ? sum + chunk.size : sum;
                           });
  }

  [[nodiscard]] std::size_t getReservedSize() const {
    std::lock_guard<std::mutex> lock(mLock);
    return std::accumulate(mAllocatedChunks.cbegin(), mAllocatedChunks.cend(),
                           std::size_t{0},
                           [](std::size_t sum, auto const& chunk) {
                             return sum + std::get<1>(chunk);
                           });
  }

  class MemorySegment {
   public:
    MemorySegment(PointerType basePointer, std::size_t size,
                  std::size_t offset = 0, PointerType tag = nullptr)
 ...
 private:
  ...
  void allocateChunk() {
    MENGINE_LOG_DEBUG("MemoryPool: Allocating %zu B", mChunkSize);
...
void MemoryPool<TAllocator>::allocateImpl(MemoryPool::PointerType* ptr,
  ...
  // When requesting 0 B, default to allocating 1 B (from "Effective C++", item
  // 51) See
  // https://stackoverflow.com/questions/2660076/returning-aligned-memory-with-new
  std::size_t const alignedRequest{
      requestedSize == 0
          ? kAlignment
          : common::ceilDiv(requestedSize, kAlignment) * kAlignment};

  MENGINE_LOG_DEBUG("MemoryPool: Requested to reserve %zu B (%zu B aligned)",
                 requestedSize, alignedRequest);

  // Finds first free segment providing sufficient space
  auto it =
      std::find_if(mMemorySegments.begin(), mMemorySegments.end(),
                   [alignedRequest](auto const& ms) {
                     return ms.tag == nullptr && ms.size >= alignedRequest;
                   });
  ...
  if (it == mMemorySegments.end()) {
    ...
    MENGINE_LOG_DEBUG(
        "MemoryPool: Needs more space to accommodate request of %zu B",
        requestedSize);
    if (mChunkSize < alignedRequest) {
      mChunkSize = alignedRequest;
      MENGINE_LOG_DEBUG("MemoryPool: Increasing chunk size to %zu B", mChunkSize);
    }
    allocateChunk();
  ...
  // Update pointer
  *ptr = static_cast<PointerType>(static_cast<std::uint8_t*>(basePointer) +
                                  offset);

  // Insert an occupied segment
  mMemorySegments.insert(
      it, MemorySegment{basePointer, alignedRequest, offset, *ptr});
}
...
void MemoryPool<TAllocator>::deallocateImpl(PointerType tag, std::size_t n) {
  ...
  MENGINE_CHECK_WITH_INFO(it != mMemorySegments.end(),
                       "MemoryPool free: Requested tag %p could not be found",
                       tag);
  ...
  if (it->size < n) {
    MENGINE_LOG_WARNING(
        "MemoryPool: Requested to free %zu B, but only %zu B available", n,
        it->size);
  }
  ...
  if (it != mMemorySegments.begin()) {
    ...
    if (previousIt->tag == nullptr &&
        previousIt->basePointer == it->basePointer) {
...
void MemoryPool<TAllocator>::logSegments() const {
  std::lock_guard<std::mutex> lock(mLock);
  MENGINE_LOG_DEBUG("MemoryPool segments:");
  for (auto ms : mMemorySegments) {
    MENGINE_LOG_DEBUG("* Segment size %zu, tag %p, basePointer %p", ms.size,
                   ms.tag, ms.basePointer);
  }
}

template <typename TAllocator>
class PoolAllocator : public BaseAllocator<PoolAllocator<TAllocator>,
                                           TAllocator::kMemoryType, false> {
  friend class BaseAllocator<PoolAllocator<TAllocator>, TAllocator::kMemoryType,
                             false>;

 public:
  using Base =
      BaseAllocator<PoolAllocator<TAllocator>, TAllocator::kMemoryType, false>;
  using PointerType = typename Base::PointerType;
...
class CudaVirtualMemoryAllocatorAdaptor
    : public BaseAllocator<CudaVirtualMemoryAllocatorAdaptor, MemoryType::kGPU,
                           /* count */ false>,
      CudaVirtualMemoryAllocator {
  // Update to MemoryCounters is done in Creator to more precisely reflect the
  // memory usage.
  using Base =
      BaseAllocator<CudaVirtualMemoryAllocatorAdaptor, MemoryType::kGPU, false>;
  friend Base;
 ...
 protected:
  void allocateImpl(PointerType* ptr, std::size_t n) const {
    this->CudaVirtualMemoryAllocator::allocate(
        ptr, n, m_engine::common::getDevice());
  }
...
//!          buffers. The template parameter TAllocator must inherit from
//!          BaseAllocator.
//!
template <typename TAllocator>
                      ...
                      TAllocator  // Inherit from TAllocator for EBO
{
 public:
  ...
  //! BufferView.
  //!
  void* data() override { return MENGINE_LIKELY(mSize > 0) ? mBuffer : nullptr; }
  ...
  [[nodiscard]] void const* data() const override {
    return MENGINE_LIKELY(mSize > 0) ? mBuffer : nullptr;
  }
 ...
 protected:
  explicit GenericBuffer(std::size_t size, std::size_t capacity,
      ...
      : TAllocator{std::move(allocator)},
        ...
        mCapacity{capacity},
        mType{type},
        mBuffer{capacity > 0 ? this->TAllocator::allocate(toBytes(capacity))
                             : nullptr} {
    MENGINE_CHECK(size <= capacity);
    MENGINE_CHECK(capacity == 0 || size > 0);
  }
...
using VirtualAddressDeviceBuffer =
    GenericBuffer<CudaVirtualMemoryAllocatorAdaptor>;

template <typename T>
std::make_unsigned_t<T> nonNegative(T value) {
  MENGINE_CHECK_WITH_INFO(value >= 0, "Value must be non-negative");
...
using VirtualAddressDeviceTensor =
    GenericTensor<CudaVirtualMemoryAllocatorAdaptor>;
