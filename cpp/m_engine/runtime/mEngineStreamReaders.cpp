...
#include "mEngineStreamReaders.h"
...
StreamReader::StreamReader(std::filesystem::path fp)
{
    ...
    MENGINE_CHECK_WITH_INFO(mFile.good(), std::string("Error opening engine file: " + fp.string()));
}
...
GDSStreamReader::GDSStreamReader(std::filesystem::path const& filePath)
{
    ...
    MENGINE_LOG_INFO("GDS driver initialization time %lld ms", elapsed_ms);
...
bool GDSStreamReader::open(std::string const& filepath)
{
    if (!initializeDriver())
    {
        MENGINE_LOG_INFO("Failed to initialize cuFile driver");
        return false;
    }
    ...
    if (ret < 0)
    {
        MENGINE_LOG_INFO("Failed to open engine file");
        return false;
    }
    ...
    {
        MENGINE_LOG_INFO("Failed to cuFileHandleRegister");
        ::close(mFd);
        return false;
    }
    return true;
...
int64_t GDSStreamReader::read(void* dest, int64_t bytes, cudaStream_t stream) noexcept
{
    ...
    {
        MENGINE_LOG_INFO("cudaPointerGetAttributes failed");
    }
...
bool GDSStreamReader::initializeDriver()
{
    ...
    if (!mCuFileLibHandle)
    {
        MENGINE_LOG_INFO("Failed to dlopen libcufile.so");
        return false;
    }
    ...
    *reinterpret_cast<void**>(&cuFileRead) = dlsym(mCuFileLibHandle, "cuFileRead");
    ...
    {
        MENGINE_LOG_INFO("Failed to dlsym libcufile.so");
        return false;
    }
    ...
    if (gdsStatus.err != CU_FILE_SUCCESS)
    {
        MENGINE_LOG_INFO("cuFileDriverOpen failed");
        return false;
    }
