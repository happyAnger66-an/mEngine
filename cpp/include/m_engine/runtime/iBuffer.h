...
#include "m_engine/common/arrayView.h"
#include "m_engine/common/dataType.h"
...
template <nvinfer1::DataType kDataType, bool kIsUnsigned = false,
          bool kIsPointer = false>
...
class BufferDataType {
 public:
  ...
  constexpr operator nvinfer1::DataType()
  ...
  [[nodiscard]] constexpr std::size_t getSize() const noexcept {
    return m_engine::common::getDTypeSize(
        static_cast<nvinfer1::DataType>(*this));
  }

  [[nodiscard]] constexpr std::size_t getSizeInBits() const noexcept {
    return m_engine::common::getDTypeSizeInBits(
        static_cast<nvinfer1::DataType>(*this));
  }
...
struct TRTDataType<std::uint32_t> {
  static constexpr auto value =
      BufferDataType{nvinfer1::DataType::kINT32, true};
};
...
struct TRTDataType<std::uint64_t> {
  static constexpr auto value =
      BufferDataType{nvinfer1::DataType::kINT64, true};
};
...
struct TRTDataType<runtime::RequestType> {
  static constexpr auto value =
      TRTDataType<std::underlying_type_t<runtime::RequestType>>::value;
};
...
struct TRTDataType<T*> {
 ...
 public:
  static auto constexpr value = BufferDataType{
      kUnderlyingType.getDataType(), kUnderlyingType.isUnsigned(), true};
};
...
std::shared_ptr<std::remove_const_t<T>> constPointerCast(
    std::unique_ptr<T, D>&& ptr) noexcept {
  return std::const_pointer_cast<std::remove_const_t<T>>(
      std::shared_ptr(std::move(ptr)));
}
...
class IBuffer {
 public:
  ...
  [[nodiscard]] virtual void* data(std::size_t index) {
    ...
    return dataPtr ? static_cast<std::uint8_t*>(dataPtr) + toBytes(index)
                   : nullptr;
  }
  ...
  static UniqueConstPtr slice(TConstPtr&& tensor, std::size_t offset,
                              std::size_t size) {
    return IBuffer::slice(constPointerCast(std::forward<TConstPtr>(tensor)),
                          offset, size);
  }
  ...
  static UniqueConstPtr view(TConstPtr&& tensor, std::size_t size) {
    return IBuffer::view(constPointerCast(std::forward<TConstPtr>(tensor)),
                         size);
  }

  //!
  //! \brief Wraps the given `data` in an `IBuffer`. The `IBuffer` will not own
  //! the underlying `data` and cannot be resized beyond `capacity`.
  //!
...
/// pointed to by the bufferPtr, or nullptr if the bufferPtr is null.
/// @tparam T The type of the underlying data.
...
/// buffer pointed to by the bufferPtr, or nullptr if the bufferPtr is null.
/// @tparam T The type of the underlying data.
/// @param bufferPtr A possibly null shared ptr.
...
/// @brief Retrieves a T typed pointer to the underlying data of the buffer
/// pointed to by the buffer pointer contained in the optionalBufferPtr, or
/// nullptr if the optional doesn't have a value.
/// @tparam T The type of the underlying data.
...
/// @brief Retrieves a T const typed pointer to the underlying data of the
/// buffer pointed to by the buffer pointer contained in the optionalBufferPtr,
/// or nullptr if the optional doesn't have a value.
/// @tparam T The type of the underlying data.
/// @param optionalBufferPtr A possibly empty optional.
/// @return A pointer to const T, possibly nullptr.
template <typename T>
...
template <typename T>
class BufferRange : public m_engine::common::ArrayView<T> {
 public:
  using Base = m_engine::common::ArrayView<T>;
