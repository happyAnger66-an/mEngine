...
{
    MENGINE_CHECK_WITH_INFO(status() == RELEASED, "virtual memory not in RELEASED status, is: %d", status());
...
{
    MENGINE_CHECK_WITH_INFO(status() == MATERIALIZED || (status() == ERRORED && mState != INVALID_STATE),
...
void OffloadConfigurator::setup(CUmemGenericAllocationHandle)
{
    if (mBackedStorage != nullptr)
    {
        if (mOndemand)
        {
            MENGINE_CU_CHECK(cuMemcpyHtoD_v2(mAddress, mBackedStorage->data(), mSize));
            mBackedStorage.reset();
        }
        else
        {
            MENGINE_CU_CHECK(cuMemcpyHtoDAsync_v2(mAddress, mBackedStorage->data(), mSize, mStream));
        }
    }
}
...
void OffloadConfigurator::teardown(CUmemGenericAllocationHandle, bool destructing)
{
    ...
    if (mBackedStorage == nullptr)
    {
        ...
        default: MENGINE_THROW("Unknown memory type: %d", static_cast<int32_t>(mBackType));
        }
    }
    // We have to synchronize here, or the memory may be unmapped before the copy operation.
    MENGINE_CU_CHECK_FREE_RESOURCE(cuMemcpyDtoH_v2(mBackedStorage->data(), mAddress, mSize));
}
    ...
    catch (...)
    {
        for (auto itRollback = begin; itRollback != it;)
        {
            ...
            catch (std::exception& e)
            {
                addBadHandle(handle);
                unsafeRemove(handle);
                MENGINE_LOG_ERROR("Additional exception thrown during rollback of materializeWithTag: %s", e.what());
            }
        }
...
void CudaVirtualMemoryAllocator::allocate(Pointer* ptr, std::size_t n, int device) const
{
    ...
    MENGINE_CU_CHECK(cuMemAddressReserve(&address, alignedSize, 0, {}, 0));
...
void CudaVirtualMemoryAllocator::deallocate(Pointer ptr, std::size_t n) const
{
    ...
    MENGINE_CU_CHECK_FREE_RESOURCE(cuMemAddressFree(address, alignedSize));
}
    ...
    std::string const& tag, CudaVirtualMemoryAllocator::RestoreMode mode, std::shared_ptr<CudaStream> backStream)
{
    ...
    MENGINE_CHECK_WITH_INFO(currentConf == bgConf,
