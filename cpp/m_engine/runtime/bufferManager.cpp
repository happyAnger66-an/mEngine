...
BufferManager::BufferManager(CudaStreamPtr stream, bool trimPool)
  ...
  MENGINE_CHECK_WITH_INFO(static_cast<bool>(mStream), "Undefined CUDA stream");
...
BufferManager::IBufferPtr BufferManager::gpu(std::size_t size,
  ...
  if (auto vmAllocator = getVirtualMemoryAllocator()) {
    return std::make_unique<VirtualAddressDeviceBuffer>(size, type,
                                                        std::move(vmAllocator));
  }
  if (static_cast<bool>(mPool)) {
    return std::make_unique<DeviceBuffer>(size, type,
                                          CudaAllocatorAsync{mStream, mPool});
  }
...
BufferManager::ITensorPtr BufferManager::gpu(nvinfer1::Dims dims,
                                             nvinfer1::DataType type) const {
  if (auto vmAllocator = getVirtualMemoryAllocator()) {
    return std::make_unique<VirtualAddressDeviceTensor>(dims, type,
                                                        std::move(vmAllocator));
  }
  if (static_cast<bool>(mPool)) {
    return std::make_unique<DeviceTensor>(dims, type,
                                          CudaAllocatorAsync{mStream, mPool});
  }
  // When memory pools are not supported, fallback to synchronous memory
  // allocations.
...
BufferManager::IBufferPtr BufferManager::gpuSync(std::size_t size,
...
BufferManager::ITensorPtr BufferManager::gpuSync(nvinfer1::Dims dims,
                                                 nvinfer1::DataType type) {
  if (auto vmAllocator = getVirtualMemoryAllocator()) {
    return std::make_unique<VirtualAddressDeviceTensor>(dims, type,
                                                        std::move(vmAllocator));
...
BufferManager::IBufferPtr BufferManager::cpu(std::size_t size,
                                             nvinfer1::DataType type) {
...
BufferManager::IBufferPtr BufferManager::pinnedPool(std::size_t size,
                                                    nvinfer1::DataType type) {
  return std::make_unique<PinnedPoolBuffer>(size, type);
}

BufferManager::ITensorPtr BufferManager::pinnedPool(nvinfer1::Dims dims,
                                                    nvinfer1::DataType type) {
...
void BufferManager::setMem(IBuffer& buffer, int32_t value) const {
  if (buffer.getMemoryType() == MemoryType::kGPU) {
    MENGINE_CUDA_CHECK(cudaMemsetAsync(
        buffer.data(), value, buffer.getSizeInBytes(), mStream->get()));
  } else {
    std::memset(buffer.data(), value, buffer.getSizeInBytes());
  }
}

void BufferManager::copy(void const* src, IBuffer& dst,
                         MemoryType srcType) const {
  if (dst.getSizeInBytes() > 0) {
    if (srcType != MemoryType::kGPU &&
        dst.getMemoryType() != MemoryType::kGPU) {
      std::memcpy(dst.data(), src, dst.getSizeInBytes());
    } else {
      MENGINE_CUDA_CHECK(m_engine::common::cudaMemcpyAsyncSanitized(
          dst.data(), src, dst.getSizeInBytes(), cudaMemcpyDefault,
          mStream->get()));
      // assert(false);
      //             MENGINE_CUDA_CHECK(false);
    }
  }
}

void BufferManager::copy(IBuffer const& src, void* dst,
                         MemoryType dstType) const {
  if (src.getSizeInBytes() > 0) {
    if (src.getMemoryType() != MemoryType::kGPU &&
        dstType != MemoryType::kGPU) {
      std::memcpy(dst, src.data(), src.getSizeInBytes());
    } else {
       MENGINE_CUDA_CHECK(m_engine::common::cudaMemcpyAsyncSanitized(
          dst, src.data(), src.getSizeInBytes(), cudaMemcpyDefault,
          mStream->get()));
    }
  }
}
...
BufferManager::IBufferPtr BufferManager::allocate(
  ...
  MENGINE_THROW("Unknown memory type");
}
...
BufferManager::IBufferPtr BufferManager::copyFrom(IBuffer const& src,
...
BufferManager::ITensorPtr BufferManager::copyFrom(ITensor const& src,
                                                  MemoryType memoryType) const {
...
std::size_t BufferManager::memoryPoolReserved() const {
  if (!static_cast<bool>(mPool)) {
    MENGINE_LOG_TRACE(
        "Operation '%s' trivially returns zero on systems without memory pool "
        "support.",
        __PRETTY_FUNCTION__);
    return 0;
  }
...
void BufferManager::memoryPoolTrimTo(std::size_t size) {
  if (!static_cast<bool>(mPool)) {
    MENGINE_LOG_TRACE(
        "Operation '%s' does not do anything on this system as it does not "
