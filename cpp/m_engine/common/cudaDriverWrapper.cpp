...
CUDADriverWrapper::CUDADriverWrapper() : handle(dllOpen(CUDA_LIB_NAME)) {
  MENGINE_CHECK_WITH_INFO(handle != nullptr,
  ...
  *reinterpret_cast<void**>(&_cuGetErrorName) =
      load_sym(handle, "cuGetErrorName");
  *reinterpret_cast<void**>(&_cuGetErrorString) =
      load_sym(handle, "cuGetErrorString");
  *reinterpret_cast<void**>(&_cuFuncSetAttribute) =
      load_sym(handle, "cuFuncSetAttribute");
  *reinterpret_cast<void**>(&_cuLinkComplete) =
      load_sym(handle, "cuLinkComplete");
  *reinterpret_cast<void**>(&_cuModuleUnload) =
      load_sym(handle, "cuModuleUnload");
  *reinterpret_cast<void**>(&_cuLinkDestroy) =
      load_sym(handle, "cuLinkDestroy");
  *reinterpret_cast<void**>(&_cuModuleLoadData) =
      load_sym(handle, "cuModuleLoadData");
  *reinterpret_cast<void**>(&_cuLinkCreate) =
      load_sym(handle, "cuLinkCreate_v2");
  *reinterpret_cast<void**>(&_cuModuleGetFunction) =
      load_sym(handle, "cuModuleGetFunction");
  *reinterpret_cast<void**>(&_cuModuleGetGlobal) =
      load_sym(handle, "cuModuleGetGlobal_v2");
  *reinterpret_cast<void**>(&_cuLibraryGetKernel) =
      load_sym(handle, "cuLibraryGetKernel");
  *reinterpret_cast<void**>(&_cuLibraryLoadData) =
      load_sym(handle, "cuLibraryLoadData");
  *reinterpret_cast<void**>(&_cuLibraryGetGlobal) =
      load_sym(handle, "cuLibraryGetGlobal");
  *reinterpret_cast<void**>(&_cuLibraryUnload) =
      load_sym(handle, "cuLibraryUnload");
  *reinterpret_cast<void**>(&_cuKernelSetAttribute) =
      load_sym(handle, "cuKernelSetAttribute");
  *reinterpret_cast<void**>(&_cuCtxGetDevice) =
      load_sym(handle, "cuCtxGetDevice");
  *reinterpret_cast<void**>(&_cuLinkAddFile) =
      load_sym(handle, "cuLinkAddFile_v2");
  *reinterpret_cast<void**>(&_cuLinkAddData) =
      load_sym(handle, "cuLinkAddData_v2");
  *reinterpret_cast<void**>(&_cuLaunchCooperativeKernel) =
      load_sym(handle, "cuLaunchCooperativeKernel");
  *reinterpret_cast<void**>(&_cuLaunchKernel) =
      load_sym(handle, "cuLaunchKernel");
  *reinterpret_cast<void**>(&_cuLaunchKernelEx) =
      load_sym(handle, "cuLaunchKernelEx");
  *reinterpret_cast<void**>(&_cuTensorMapEncodeTiled) =
      load_sym(handle, "cuTensorMapEncodeTiled");
  *reinterpret_cast<void**>(&_cuMemcpyDtoH) =
      load_sym(handle, "cuMemcpyDtoH_v2");
  *reinterpret_cast<void**>(&_cuDeviceGetAttribute) =
...
CUresult CUDADriverWrapper::cuGetErrorName(CUresult error,
                                           char const** pStr) const {
  return (*_cuGetErrorName)(error, pStr);
}

CUresult CUDADriverWrapper::cuGetErrorString(CUresult error,
                                             char const** pStr) const {
...
CUresult CUDADriverWrapper::cuModuleGetGlobal(CUdeviceptr* dptr, size_t* bytes,
                                              CUmodule hmod,
                                              char const* name) const {
  return (*_cuModuleGetGlobal)(dptr, bytes, hmod, name);
}

CUresult CUDADriverWrapper::cuLibraryGetKernel(CUkernel* pKernel,
                                               CUlibrary library,
                                               char const* name) const {
  return (*_cuLibraryGetKernel)(pKernel, library, name);
}

CUresult CUDADriverWrapper::cuLibraryLoadData(
    CUlibrary* library, void const* code, CUjit_option* jitOptions,
    void** jitOptionsValues, unsigned int numJitOptions,
    CUlibraryOption* libraryOptions, void** libraryOptionValues,
    unsigned int numLibraryOptions) const {
  return (*_cuLibraryLoadData)(library, code, jitOptions, jitOptionsValues,
                               numJitOptions, libraryOptions,
                               libraryOptionValues, numLibraryOptions);
}

CUresult CUDADriverWrapper::cuLibraryGetGlobal(CUdeviceptr* dptr, size_t* bytes,
                                               CUlibrary library,
                                               char const* name) const {
...
CUresult CUDADriverWrapper::cuKernelSetAttribute(CUfunction_attribute attrib,
...
CUresult CUDADriverWrapper::cuLinkAddFile(CUlinkState state,
                                          CUjitInputType type, char const* path,
...
std::string stringify_launch_config(CUlaunchConfig const& config) {
  ...
  ss << "Stream: " << (config.hStream ? "Custom" : "Default") << " (0x"
     << std::hex << reinterpret_cast<uintptr_t>(config.hStream) << ")\n";
  ...
  for (uint i = 0; i < config.numAttrs; ++i) {
    ...
    switch (attr.id) {
      case CU_LAUNCH_ATTRIBUTE_CLUSTER_DIMENSION:
        ss << "Cluster Dimension: (" << attr.value.clusterDim.x << ", "
...
CUresult CUDADriverWrapper::cuLaunchKernelEx(CUlaunchConfig const* config,
                                             CUfunction f, void** kernelParams,
                                             void** extra) const {
  MENGINE_LOG_DEBUG("Launch config: %s",
                    stringify_launch_config(*config).c_str());
  MENGINE_CHECK_DEBUG_WITH_INFO(
...
CUresult CUDADriverWrapper::cuTensorMapEncodeTiled(
    ...
    cuuint32_t tensorRank, void* globalAddress, cuuint64_t const* globalDim,
    cuuint64_t const* globalStrides, cuuint32_t const* boxDim,
    cuuint32_t const* elementStrides, CUtensorMapInterleave interleave,
    CUtensorMapSwizzle swizzle, CUtensorMapL2promotion l2Promotion,
    CUtensorMapFloatOOBfill oobFill) const {
  return (*_cuTensorMapEncodeTiled)(tensorMap, tensorDataType, tensorRank,
...
CUresult CUDADriverWrapper::cuDeviceGetAttribute(int* pi,
