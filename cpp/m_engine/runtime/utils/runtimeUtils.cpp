...
namespace m_engine::runtime::utils
{
    ...
    int initDevice(WorldConfig const &worldConfig)
    {
        ...
        MENGINE_CUDA_CHECK(cudaSetDevice(device));
        return device;
    }
    ...
    std::vector<uint8_t> loadEngine(std::string const &enginePath)
    {
        ...
        MENGINE_CHECK_WITH_INFO(engineFile.good(), std::string("Error opening engine file: " + enginePath));
        ...
        MENGINE_CHECK_WITH_INFO(engineFile.good(), std::string("Error loading engine file: " + enginePath));
        return engineBlob;
    }
