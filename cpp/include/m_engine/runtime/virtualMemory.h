...
#include "m_engine/runtime/cudaEvent.h"
...
class CUDAVirtualMemoryChunk {
 public:
   ...
   * CUmemGenericAllocationHandle, either by creating one locally, or importing
   * one from remote.
   */
  struct Creator {
    Creator() = default;
    ...
    // CUDAVirtualMemoryChunk is being destructed.
    virtual CUmemGenericAllocationHandle create() = 0;
    virtual void release(CUmemGenericAllocationHandle handle,
                         bool destructing) = 0;
  };
  ...
  struct Configurator {
    ...
    virtual void teardown(CUmemGenericAllocationHandle handle,
                          bool destructing) = 0;
  };
   ...
   *
   * Calls creator.create(), and then configurator.setup() for each configurator
   * in order.
   *
   ...
   * throws. Will be called automatically by destructor if necessary.
   *
   * Calls configurator.teardown() for each configurator that setup() succeed in
   * materialize() in reversed order, and then creator.release().
   *
   * Never stops early upon exception. The last thrown exception will be
   * propagated, and others logged.
   */
  void release() { _release(false); }
  ...
  CUDAVirtualMemoryChunk(CUDAVirtualMemoryChunk&& other) noexcept {
    ...
    mState = other.mState;
    new (&other)
        CUDAVirtualMemoryChunk;  // Put other into default constructed state
  }
  ...
  virtual ~CUDAVirtualMemoryChunk() {
    ...
    // exception. If release() is already called by the user, whether succeed or
    // threw an exception, we shouldn't call release() again.
   ...
   */
  explicit operator bool() const noexcept { return mCreator != nullptr; }
...
struct LocalCreator : CUDAVirtualMemoryChunk::Creator {
  LocalCreator(CUmemAllocationProp const& prop, size_t size)
      : mProp(prop), mSize(size) {}

  CUmemGenericAllocationHandle create() override {
    CUmemGenericAllocationHandle handle{};
    MENGINE_CU_CHECK(cuMemCreate(&handle, mSize, &mProp, 0));
    if constexpr (count) {
  ...
  void release(CUmemGenericAllocationHandle handle, bool destructing) override {
    MENGINE_CU_CHECK_FREE_RESOURCE(cuMemRelease(handle));
    if constexpr (count) {
...
struct UnicastConfigurator : CUDAVirtualMemoryChunk::Configurator {
  UnicastConfigurator(CUdeviceptr address, size_t size,
                      CUmemAccessDesc const& desc)
      : mAddress(address), mSize(size), mDesc(desc) {}

  void setup(CUmemGenericAllocationHandle handle) override {
    MENGINE_CU_CHECK(cuMemMap(mAddress, mSize, 0, handle, 0));
    MENGINE_CU_CHECK(cuMemSetAccess(mAddress, mSize, &mDesc, 1));
  }

  void teardown(CUmemGenericAllocationHandle, bool) override {
    MENGINE_CU_CHECK_FREE_RESOURCE(cuMemUnmap(mAddress, mSize));
  }
...
struct MulticastConfigurator : CUDAVirtualMemoryChunk::Configurator {
  void setup(CUmemGenericAllocationHandle handle) override {
    MENGINE_CU_CHECK(
        cuMulticastBindMem(mMulticast, 0, handle, mBindOffset, mSize, 0));
  }

  void teardown(CUmemGenericAllocationHandle, bool) override {
    MENGINE_CU_CHECK_FREE_RESOURCE(
        cuMulticastUnbind(mMulticast, mDevice, 0, mSize));
  }
...
struct MemsetConfigurator : CUDAVirtualMemoryChunk::Configurator {
  MemsetConfigurator(CUdeviceptr address, size_t size, uint8_t value,
                     CUstream stream)
      : mAddress(address), mSize(size), mStream(stream), mValue(value) {}

  void setup(CUmemGenericAllocationHandle) override {
    if (mFirstTime) {
      mFirstTime = false;
    } else {
      MENGINE_CU_CHECK(cuMemsetD8Async(mAddress, mValue, mSize, mStream));
    }
  }
 ...
 * storage when teardown, and restore the content on the following setup.
 */
struct OffloadConfigurator : CUDAVirtualMemoryChunk::Configurator {
  OffloadConfigurator(CUdeviceptr address, size_t size, MemoryType backType,
                      CUstream stream, bool ondemand = false)
      : mAddress(address),
        mSize(size),
        mBackType(backType),
        mStream(stream),
...
class CudaVirtualMemoryManager {
 public:
   ...
   */
  void add(uintptr_t handle, std::string tag,
  ...
  void add(uintptr_t handle, std::string tag,
    ...
    add(handle, tag, std::move(creator),
        {std::forward<Configurators>(configurators)...});
  }
   ...
   * selected objects. The last exception thrown by
   * `CUDAVirtualMemoryChunk::release` will be rethrown, and others will be
   * logged.
   *
   * If any CUDAVirtualMemoryChunk threw an exception during `release`, it will
   * be removed from the manager. Call `retrieveBadHandles` to retrieve handles
   * of all CUDAVirtualMemoryChunk that got removed due to exception.
   */
   ...
   * @return Number of objects selected.
   *
   * This function will stop at the first `CUDAVirtualMemoryChunk::materialize`
   * that throws exception, and attempt to roll back previous successful
   * `materialize` by calling `release`. The exception thrown by
   * `CUDAVirtualMemoryChunk::materialize` will be rethrown, and any exception
   * thrown by `release` will be logged.
   *
   * If any CUDAVirtualMemoryChunk threw an exception during `materialize` or
   * `release`, it will be removed from the manager. Successfully roll backed
   * CUDAVirtualMemoryChunk will not be removed. Call `retrieveBadHandles` to
  ...
  /**
   * Retrieve handles of all CUDAVirtualMemoryChunk that got removed due to
   * exception and reset the list. The returned list may not include all removed
   * CUDAVirtualMemoryChunk handles if OOM happened. This method is only for
   * diagnostic purpose, and should not be called concurrently with other
   * methods.
 ...
 public:
  ...
  class Configuration {
    ...
    friend void setVirtualMemoryAllocator(
        std::string const& tag, RestoreMode mode,
   ...
   public:
     ...
     */
    Configuration(CudaVirtualMemoryManager& manager, std::string tag,
                  RestoreMode mode, CudaStreamPtr backStream)
        : mManager(manager),
          mTag(std::move(tag)),
    ...
    [[nodiscard]] std::size_t aligned(std::size_t n, int device = 0) {
      // Lazy loading the alignment, since CUDA driver may yet to be initialized
      // when Configuration is constructed. We have one process for each GPU so
      // caching the value is fine.
      constexpr std::size_t loading = std::numeric_limits<std::size_t>::max();
      std::size_t alignment = 0;
      if (mAlignment.compare_exchange_strong(alignment, loading,
                                             std::memory_order_relaxed)) {
        std::size_t gpuAlignment = 1;
        CUmemAllocationProp const prop{CU_MEM_ALLOCATION_TYPE_PINNED,
                                       CU_MEM_HANDLE_TYPE_NONE,
                                       {
                                           CU_MEM_LOCATION_TYPE_DEVICE,
                                           device,
                                       }};
        MENGINE_CU_CHECK(cuMemGetAllocationGranularity(
            &gpuAlignment, &prop, CU_MEM_ALLOC_GRANULARITY_RECOMMENDED));
    ...
    // Background configuration, used to indicate no virtual memory allocator is
    // explicitly configured by the user.
    static Configuration backgroundConfiguration;

   private:
    Configuration(CudaVirtualMemoryManager& manager, std::string tag,
                  RestoreMode mode, CudaStreamPtr backStream, bool background)
  ...
  explicit operator bool() const noexcept { return !mConfig->mBackground; }
...
void setVirtualMemoryAllocator(std::string const& tag,
