...
class CudaStream {
 public:
  ...
  //! for a list of valid flags that can be passed. \param priority Priority of
  //! the stream. Lower numbers represent higher priorities. See
  ...
  explicit CudaStream(unsigned int flags = cudaStreamNonBlocking,
                      int priority = 0)
      : mDevice{m_engine::common::getDevice()} {
    cudaStream_t stream;
    MENGINE_CUDA_CHECK(::cudaStreamCreateWithPriority(&stream, flags, priority));
    MENGINE_LOG_TRACE("Created stream %p", stream);
  ...
  explicit CudaStream(cudaStream_t stream)
      : CudaStream{stream, m_engine::common::getDevice(), false} {}
  ...
  void synchronize() const { MENGINE_CUDA_CHECK(::cudaStreamSynchronize(get())); }

  //! \brief Record an event on the stream.
  void record(CudaEvent::pointer event) const {
    MENGINE_CUDA_CHECK(::cudaEventRecord(event, get()));
  }
  ...
  void wait(CudaEvent::pointer event) const {
    MENGINE_CUDA_CHECK(::cudaStreamWaitEvent(get(), event));
  }
 ...
 private:
   ...
   public:
    ...
    constexpr void operator()(cudaStream_t stream) const {
      if (mOwnsStream && stream != nullptr) {
        MENGINE_CUDA_CHECK(::cudaStreamDestroy(stream));
        MENGINE_LOG_TRACE("Destroyed stream %p", stream);
      }
    }
  ...
  using StreamPtr =
      std::unique_ptr<std::remove_pointer_t<cudaStream_t>, Deleter>;
