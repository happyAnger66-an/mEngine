...
#include "m_engine/runtime/tensorView.h"
...
ITensor::UniquePtr ITensor::slice(SharedPtr tensor, Shape const& offsetDims, ITensor::DimType64 size)
{
    ...
    MENGINE_CHECK(offsetDims.nbDims >= 0);
    MENGINE_CHECK(shape.nbDims >= offsetDims.nbDims);
    MENGINE_CHECK(size >= 0);

    Shape strides = ITensor::strides(shape);
    DimType64 offset{0};
    for (SizeType32 di = 0; di < offsetDims.nbDims - 1; di++)
    {
        MENGINE_CHECK(0 <= offsetDims.d[di] && offsetDims.d[di] < shape.d[di]);
        offset += offsetDims.d[di] * strides.d[di];
    }

    if (MENGINE_LIKELY(offsetDims.nbDims > 0))
    {
        MENGINE_CHECK(offsetDims.d[offsetDims.nbDims - 1] + size <= shape.d[offsetDims.nbDims - 1]);
        offset += offsetDims.d[offsetDims.nbDims - 1] * strides.d[offsetDims.nbDims - 1];
    }
    else
    {
        MENGINE_CHECK(size >= 0 && size <= 1);
        MENGINE_CHECK(shape.nbDims == 0 ? size == 0 : true);
    }
...
{
    MENGINE_CHECK_WITH_INFO(dims.size() <= nvinfer1::Dims::MAX_DIMS, "Number of dimensions is too large");
    nvinfer1::Dims shape{};
...
ITensor::UniquePtr ITensor::wrap(void* data, nvinfer1::DataType type, nvinfer1::Dims const& shape, std::size_t capacity)
{
    ...
    MENGINE_CHECK_WITH_INFO(size <= capacity, "Requested size is larger than capacity");
    ...
    case MemoryType::kUVM:
        ...
        break;
    default: MENGINE_THROW("Invalid memory type."); break;
    }
    return result;
}
...
{
    MENGINE_CHECK_WITH_INFO(0 < shape.nbDims, "Cannot squeeze 1-dimensional tensor");
    MENGINE_CHECK_WITH_INFO(
        dim < shape.nbDims, tc::fmtstr("Invalid index %d, tensor has %d dimensions", dim, shape.nbDims));
    MENGINE_CHECK_WITH_INFO(shape.d[dim] == 1, "Can only squeeze dimension of size 1");
...
{
    MENGINE_CHECK_WITH_INFO(shape.nbDims < Shape::MAX_DIMS, "Too many dimensions to unsqueeze");
    MENGINE_CHECK_WITH_INFO(
...
{
    MENGINE_CHECK_WITH_INFO(tensor.getDataType() == TRTDataType<typename std::remove_cv<T>::type>::value,
...
std::ostream& m_engine::runtime::operator<<(std::ostream& out, ITensor const& tensor)
{
